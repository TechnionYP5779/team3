...
@SuppressWarnings("static-method") public class RealListTest {
  @Test public void MajorGeneralTest() {
    ...
    assert ite.next().first.equals(box.it(1.0));
    assert ite.next().first.equals(box.it(2.0));
    final Iterator<Pair<Double, Double>> ite2 = li.iteratorY();
    assert ite2.next().second.equals(box.it(1.0));
    assert ite2.next().second.equals(box.it(1.3));
